...
In the light of these results, we might be tempted to search for expressions for Cantor-type functions, since having a bijection would allow us to have a direct mapping between the square and the segment populations. Unfortunately, these functions are discontinuous which breaks the regularity of the kernel.

Peano functions seem like good candidates for the mapping $S^{-1}$, and are extensively studied in the litterature (we refer for instance to [@sagan_space-filling_1994;@Sagan1993]). They have the nice property that they are continuous, and are surjective, so we the segment populations are able to "reach" the entirety of the square populations. All things considered, non-injectivity is not a problem, since it just means that two segment populations can "refer" the same square population, which does not break the regularity of the kernel, since the two segments populations can just have their own neighbourhoods. Furthermore, we will see that in our examples, the non-injectivity can be ignored because the mapping is quasi-bijective.
...
The Z-mapping however seems to converge to a fractal curve. Numerically of course, there seem to be infinite number of discontinuities, but the size of these discontinuities is overwhelmingly small, and *on average* the size of these numerical disconuities vanishes as $n\to\infty$. We don't go further into detail about this in this section (in @sec-zcurve-lebesgue we develop on the convergence to a space-filling fractal curve), but this gives some intuition for the arguments presented in @sec-locality.
...
[^zmapping-notbijective]: Despite what can often be found in online resources, the Z-mapping is *not* a bijection because that would violate Netto's theorem. The rew_ason for this is the ambiguity in the writing of binary expansions, and in @sec-zcurve-noninjective we give a proof that the Z-mapping is not injective in the $n\to\infty$ limit. However the Z-mapping is *quasi-bijective*, in the sense that the set of points for which the Z-mapping is not injective has zero measure, and can therefore be ignored.
