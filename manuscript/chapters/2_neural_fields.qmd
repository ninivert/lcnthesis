...
The geometric view of these networks is that the recurrent currents lie in a $p$-dimensional subspace of $\R^N$ spanned by the linearly independant vectors $\{\vec{F_1}, \cdots, \vec{F_\mu},\cdots, \vec{F_p}\}$ (where each $\vec{F_\mu}=(F_{\mu 1}, \cdots F_{\mu N})$), which therefore define a "natural" linear embedding (in the sense of @JazOst21) of the neural population activity.
...
### Gaussian low-rank network of neurons
...
We note that contrary to the model introduced in @schmutz2023convergence, which excludes the self-connection term by effectively setting $J_{ii} = 0$, we do not do this, because it introduces a vanishing correction of order $\bO(1/N)$.
...
```{python}
#| label: fig-spectrum
#| fig-cap: Spectrum of $K$ for $N=1024$ and $p=5$. Numerical estimations (black, every line represents the sorted spectrum of one random sample of $K$, 20 samples total) of the eigenvalues are close to the analytical ($N \to \infty$) derivation (red, only eigenvalues $\lambda_\mu$). Eigenvalues above the dotted gray line (positive values) correspond to unstable fixed points, and below (negative values) correspond to stable fixed points. The first $p$ eigenvalues correspond to $\lambda_\mu$ ($\mu\in\{1,\cdots,p\}$), and the remaining eigenvalues correspond to $\lambda_\perp=-1$.
#| fig-subcap:
...
N = 1024
p = 5
numeig = 20  # number of stored numerical eigenvalues
dsigmoid = lambda x: sigmoid(x) * (1 - sigmoid(x))

def plot_spectrum(hstar_str: str):
	nruns = 20
	eig_nums = np.zeros((nruns, numeig))

	for i, seed in enumerate(range(nruns)):
		params = LowRankRNNParams.new_valentin(p=p, N=N, phi=sigmoid, I_ext=zero, exclude_self_connections=False, random_state=seed)
		if hstar_str == 'pattern': hstar = params.F
		elif hstar_str == 'zero': hstar = np.zeros_like(params.F)
		dphi = dsigmoid(hstar)
		J = np.einsum('im,jm->ij', params.F, params.G * dphi) / params.N - np.eye(params.N)
		eig_nums[i] = np.sort(np.linalg.eigvals(J).real)[::-1][:numeig]

	eig_num = eig_nums.mean(axis=0)
	eig_num_std = eig_nums.std(axis=0, ddof=1) / np.sqrt(nruns)  # std of the mean estimator
	...
	# ax.errorbar(np.arange(8), eig_num[:8], yerr=eig_num_std[:8], marker='o', clip_on=False, linestyle=':')
	ax.plot(eig_nums[:, :8].T, marker='.', color='k', alpha=0.2, clip_on=False, linestyle=':')
	ax.plot([7,8,8.5], eig_num[8:11], linestyle='--', color='k', clip_on=False)  # dashed line "fadeout"
	# analytical estimations
	ax.plot(np.arange(params.p), np.full(params.p, eig_ana), color='r', marker='o', linestyle=':', clip_on=False)
	ax.set_ylim((-1, 0.5))
	ax.set_xlim((0, 8.5))
...
#| label: fig-p1
#| layout-nrow: 2
#| fig-cap: Simulation of the Gaussian network of neurons with $p=1$. Every neuron is represented by a dot, of which the height represents its activity level, and the colorbar doubles as a y-axis. The first and the last frame of the animation are shown, [available here](https://ninivert.github.io/lcnthesis/chapters/figures/embedding_p=1.mp4).
#| fig-subcap:
#|   - First frame
#|   - Last frame
#| echo: false
...
display(Image('figures/embedding_p=1.mp4-1.jpg'))
display(Image('figures/embedding_p=1.mp4-2.jpg'))
```
:::
...
#| label: fig-p2
#| layout-ncol: 2
#| fig-cap: Simulation of the Gaussian network of neurons with $p=2$. Every neuron is represented by a dot, of which the height represents its activity level. Projections are shown by gray points. The first and the last frame of the animation are shown, [available here](https://ninivert.github.io/lcnthesis/chapters/figures/embedding_p=2.mp4).
#| fig-subcap:
#|   - First frame
#|   - Last frame
#| echo: false
# Note: I have to do this wizardry because Quarto *really* doesn't seem to like nested divs with conditional content...
from IPython.display import Image
display(Image('figures/embedding_p=2.mp4-1.jpg'))
display(Image('figures/embedding_p=2.mp4-2.jpg'))
```
:::
...
\mathcal{N}^p(\d \vec y) &= \frac{\mathrm{exp}(\frac12\sum_{\mu=1}^p y_\mu^2)}{(2\pi)^{p/2}} \mathrm{d} y_1 \cdots \mathrm d y_p.
\end{aligned}
$$ {#eq-nf-gaussian-p}
...
Due to the formulation of the neural field weighted by a probability distribution $\rho(y_1, \cdots, y_p)$, a natural way to estimate the integral is by Monte Carlo integration.
The essence of this method is to use the Central Limit Theorem, where we take $N$ independent samples $\vec{y_i} = (y_{1i}, \cdots, y_{pi})$ from the distribution $\rho$, and use them to estimate the integrand $I(\vec z, \vec y)$.
...
::: {.content-visible when-format="pdf"}
```{python}
#| label: fig-cycling-anim
#| layout-nrow: 2
#| fig-cap: A cycling neural field with $\delta=6$ and initial condition $h(z_1, z_2, t < 0) = z_1$. The latent trajectory is also computed, and demonstrates the oscillatory behaviour. The first and the last frame of the animation are shown, [available here](https://ninivert.github.io/lcnthesis/chapters/figures/embedding_p=2_cycling.mp4).
#| fig-subcap:
#|   - First frame
#|   - Last frame
#| echo: false
# Note: I have to do this wizardry because Quarto *really* doesn't seem to like nested divs with conditional content...
from IPython.display import Image
display(Image('figures/embedding_p=2_cycling.mp4-1.jpg'))
display(Image('figures/embedding_p=2_cycling.mp4-2.jpg'))
```
:::
