...
Let us for a moment consider another type of model for networks of neurons, that of homogeneous population dynamics, that describes how $N_\text{pop}$ different populations interact with each other. These types of models emerge when networks of neurons exhibit a finite number of "neuron types". We then group neurons of the same type, and make the approximation that they have identical potentials and the same interaction weights with other neurons. In effect, every population then represents an infinite number of discrete identical neurons.
...
When taken in isolation, the previous developments might sound a little bit silly, though they serve to give an impression of the problem at hand, and an interpretation for the rest of this work. For a finite number of populations $N_\text{pop}$, the population dynamics is well-defined, however, what happens as $N_\text{pop} \to \infty$ ? The core of the question lies in the fact that we can easily *enumerate* a finite number of populations, but the sum $\sum_{b=1}^{N_\text{pop}}$ must have a clearly defined interpretation when this number tends to infinity.

In light of the toy model presented in @sec-nf, we can reinterpret the density $\rho$ of the neural field equation. Every point $\vec z \in \R^p$ (let us consider $p=2$ for the rest) describes an infinite population of homogeneous neurons, associated with a probability (density) $\rho(\vec z)$. Therefore, in order to write an equivalent neural field in $[0,1]$, we need a way to *enumerate* the populations. This is done using a mapping $S : [0,1]^2 \mapsto [0,1]$, where we traverse the $[0,1]^2$ space by following the order in the image $[0,1]$.

Additionally, the notion of population dynamics helps give another interpretation of the grid integration method given in @sec-gridmethod. When we discretize the $[0,1]^2$ space, we are effectively performing a coarse-graining approximation and simulating a finite number of populations. For every bin localized by $(v_1, v_2)$, we are sampling the connectivity kernel $w_U(\cdot, (v_1, v_2))$ and 2D neural field $h_U((v_1, v_2), t)$ (see @fig-grid-sampling). In the rest of the chapter, we refer to these 2D bins as "square populations", and to the corresponding 1D bins obtained by the mapping $S$ as "segment populations".

![(TODO, reorder args of $w_U$) Every point of the grid effectively performs coarse-graining, and defines a point where we sample the connectivity kernel $w_U$ and the 2D neural field $h_U$](figures/photo_2023-06-10_21-13-23.jpg){#fig-grid-sampling}
...
We start by writing the neural field equation in the $[0,1]$ space. For this, we define a measurable and bijective mapping $S : [0,1]^2 \rightarrow [0, 1]$. Furthermore, we let $\lambda : [0,1]^2 \rightarrow [0, \infty]$ be a measure on the unit square (for instance, the Lebesgue measure, which plays the role of a uniform distribution). Taking the composition $\lambda \circ S^{-1}$ then defines a measure on $[0,1]$, which keeps track of how 2D surfaces are transformed to 1D lengths through the mapping $S$.

The connectivity kernel $\tilde w$ defines the interactions between the populations in the $[0,1]$ embedding, which we define relating to the uniform kernel on $[0,1]^2$ as :
...
For these reasons, we can see why the composition of $w_U$ with a possibly non-differentiable mapping $S$ might be problematic. Since the only thing we know about $S$ is that it is measurable and bijective, *there is no guarantee that the regularity of the kernel is conserved through composition with the mapping $S$.*
...
In other words, $P^{-1}(\alpha)$ defines a vertical line in the $[0,1]^2$ embedding. When we then take the composition $w_U \circ P^{-1}$, the regularity is destroyed, in the sense that the same population $\alpha$ in the 1D embedding will "sense" all the connectivity kernels on the vertical line.
...
However, this again fails because the joint cumulative function is not bijective, and isolines of the CDF define diagonals in the uniform embedding space $[0,1]^2$.[^jointcdf]
...
Since in the composition $w_U \circ S^{-1}$ we work with the inverse mapping, it might be judicious to directly consider functions from $[0,1]$ into $[0,1]^2$. We have already established that there is no homeomorphism between those spaces, and need to relax one of the conditions.
...
In light of these results, we might be tempted to search for expressions of Cantor-type functions, since having a bijection would allow us to have a direct mapping between the square and the segment populations. Unfortunately, these functions are discontinuous which breaks the regularity of the kernel.

Peano functions seem like good candidates for the mapping $S^{-1}$, and are extensively studied in the literature around space-filling curves (we refer for instance to [@sagan_space-filling_1994]). They have the nice property that they are continuous and surjective, so we the segment populations can "reach" the entirety of the square populations. All things considered, non-injectivity is not a problem, since it just means that two segment populations can "refer" to the same square population, which does not break the regularity of the kernel, since the two segment populations can just have their own neighborhoods. Furthermore, we will see that in our examples, the non-injectivity can be ignored because the mapping is "quasi-bijective".

![Non-injectivity of $S^{-1}$ means that two segment populations can refer to the same square population.](figures/photo_2023-06-12_00-46-26.jpg)
...
The intuition and illustration @fig-recurse-binary behind this expansion is that we locate each component by recursively splitting the segment in two sub-segments of equal length. At every step, taking the left or right segments defines the sequence of binary digits.
<!-- Then combine the bits of $v^{(n)}_1$ and $v^{(n)}_2$ to get the position in the $[0,1]^2$ embedding. -->

![(TODO rename bins to segments) Splitting $[0,1]$ recursively gives the binary bit expansion of $v_1$](figures/photo_2023-06-10_17-21-28.jpg){#fig-recurse-binary}
...
These simplifications might seem too restrictive, however, we will show in the rest of this thesis that this formulation is flexible enough to express mappings that have good properties such as "locality", all while being simple enough to help build intuitions.

TODO NOTE : might need to talk about how $S(v^{(n)}_1,v^{(n)}_2) = S^n(v^{(n)}_1,v^{(n)}_2)$ ? so basically the limit $n\to\infty$ is the limit where we take infinite precision in the input. Or we can drop the $n$ on the inputs, $\alpha^{(n)} = S^n(v^{(n)}_1,v^{(n)}_2) = S^n(v_1,v_2)$ (i think this true just for pointwise convergent mappings)

TODO NOTE : talk about how we center the bins instead of taking the corners. this is not super important in the $n\to\infty$ limit, but numerically at 0 and 1 the $\mathrm{CDF}^{-1}$ goes to infty. at infinity, the connectivity kernel is infinite but we have zero neurons (zero measure, linear growth of kernel vs exponential tail of the gaussian) : effectively the recurrent current is zero and we can safely ignore these effects.
...
One way of ordering the 2D texture data in the 1D memory is by using the Z-order curve (also known as "Morton mapping" or "Lebesgue curve" @Sagan1994). In this way, points close in 1D *tend* to be tightly packed in 2D. We can define the Z-order curve (Z-mapping) in the following way:[^zcurve]

[^zcurve]: The Z-curve is often defined in a transpose manner, putting first the bits of $v_2$ (vertical component) then the bits of $v_1$, instead of $v_1$ then $v_2$. By doing this, we get the "upright" Z shapes, as [shown here](https://en.wikipedia.org/wiki/Z-order_curve#/media/File:Four-level_Z.svg).
...
@fig-anti-z gives some insight into the problem of this mapping. For a fixed input $(v_1,v_2)$, we see that the output point position jumps around when $n$ is changed, and that these jumps seem to always be of order 1. The intuition behind pointwise convergence is that as $n$ increases, the image of a point by $S^n$ is continually refined.
...
#### Conservation of bijectivity in the $n\to\infty$ limit {#sec-ninfty-bijection}
...
The Z-mapping seems to converge to a fractal curve. Numerically, there seems to be an infinite number of discontinuities, but the size of these discontinuities is overwhelmingly small, so that *on average* they vanish as $n\to\infty$. We won't go further into detail about this in this section (in @sec-zcurve-lebesgue we develop the convergence to a space-filling fractal curve), but this gives some intuition for the arguments presented in @sec-locality.

We finish this discussion on the limit of the sequence of mappings by asking if there are other mappings similar to the Z-mapping, in the sense that they are pointwise convergent and the limit is bijective.[^zmapping-notbijective] In an informal way, we established two conditions for this in the context of the mappings described in @sec-mappings-binary:

[^zmapping-notbijective]: Despite what can often be found in online resources, the Z-mapping is *not* a bijection because that would violate Netto's theorem. The reason for this is the ambiguity in the writing of binary expansions, and in @sec-zcurve-noninjective we give proof that the Z-mapping is not injective in the $n\to\infty$ limit. However, the Z-mapping is *quasi-bijective*, in the sense that the set of points for which the Z-mapping is not injective has zero measure, and can therefore be ignored.
...
### Practical aspects of implementing the mappings

In the previous sections, we have discussed what mappings have good properties, and have given a few hints as to how we can implement them numerically. Since the mappings we presented can be written as a binary expansion, they naturally lend themselves to numerical approximations. Furthermore, this is made especially easy since the mappings we defined simply are permutations of bits (in the $n < \infty$ case), and so we can write a one-to-one correspondence between unsigned integers.

Without going into too much detail on the programming, we have implemented the mappings as all inheriting from the same abstract class, and each mapping simply has to define a method `indices_to_indices2d(indices: array[int, 1]) -> array[int, 2]` to go from 1D to 2D ($S^{-1}$), and a method `indices2d_to_indices(indices2d: array[int, 2]) -> array[int, 1]` to go from 2D to 1D.[^indices] The abstract class simply defines additional quality-of-life functionality, which makes use of these two methods.

[^indices]: In the source code, only the method `indices(v12: array[float, 2]) -> array[int, 1]` is defined. This is because some mappings are implemented (for instance, the `RecursiveLocalMapping`, that we don't discuss here), which don't lend themselves to the simple permutation of bits formulation.

### Numerical approximations of a fractal structure

In the continuation of the simulation methods established in the @sec-simnf, we adopt a grid discretization scheme, as it naturally lends itself to be applied with the mappings. We however note that using binning methods, the Monte-Carlo simulation can still be applied. The intuition behind this is that sampled populations inside a small square of size $2^{-n} \times 2^{-n}$ give rise to the notion of an "average connectivity kernel", and in our case the notion of "mean patterns". More details are given in @sec-mean-patterns.
...
1. Create a grid of $2^n \times 2^n = 4^n$ squares of size $2^{-n} \times 2^{-n}$ in the $[0,1]\times[0,1]$ embedding. Each square is located by a position $\vec{v_i} = (v_{1,i},v_{2,i}),\,i\in\{1,\cdots\,2^n\}$.
2. Compute a discretized version of the 1D connectivity kernel
   ...
   \end{aligned}
   $$
   at every square location (where we denote for simplicity of notation $S(i)=S(\vec{v_i})$).
3. Simulate the neural field in $[0,1]$ on the corresponding discretized space with discretization step $4^{-n}$.
...
In some sense, we would like to "take into account" the fact that a mapping of the square populations has been performed. Let us consider what happens to the mapping when $n$ is large, and in particular how $S^{n/2}$ and $S^{n}$ behave. Since the mappings are pointwise convergent and we are considering large $n$, we argue that both $S^{n/2}$ and $S^{n}$ are good approximations of the true mapping $S$. More importantly, $S^{n/2}$ can be considered as an approximation of $S^{n}$. This is interesting from a numerical standpoint since it is feasible to compute $S^{n}$, and the approximation $S^{n/2}$ can be obtained by "coarse-graining" $S^n$. Another way of thinking about this is that $S^n$ is considered to be the "mathematically exact" fractal structure and $S^{n/2}$ the numerical approximation.

The way we go from $S^n$ to $S^{n/2}$ is by averaging the segment populations by batches of $2^n$, as schematized in @fig-averaging. We call this operation "downsampling" the $S^n$ mapping.

![(TODO) Averaging $2^n$ of the $4^n$ segments populations of $S^n$ results in the $2^n = 4^{n/2}$ segments populations of $S^{n/2}$](figures/photo_2023-06-13_12-15-32.jpg){width=400px #fig-averaging}

The geometrical intuition we can give for this is how the averaging of the segment populations leads to sampling the connectivity kernel at the "average" positions in the $[0,1]^2$ embedding. In @fig-averaging-geom, we colour-code the square populations corresponding to the segment populations before averaging.

::: {#fig-averaging-geom layout-ncol=2}

![Z-mapping](figures/averaging_z.png)

![Column mapping](figures/averaging_column.png)

(TODO) The corresponding square populations corresponding to the segment populations before downsampling.
:::
...
TODO : we run simulations and compare latent trajectories. first show only column and z-mapping. add quick intro to diagonal, col/row, reclocal, mention cannot be expressed by bit permutation. diagonal, col/row do not work, despite the non-zero field and recurrent currents. reclocal does work. intuition is that the recursive approach (zmapping, reclocal) seems to give locality

![(TODO animation) Simulations of downsampling with different mappings from the $p=2$ embedding.](figures/coarsegraining.png){width=500px}

### Iterative application of $S$ : from $[0,1]^3$ to $[0,1]$ {#sec-simulations-3d-2d-1d}

TODO : take cycling p=3

#### Continuity on one axis and fractal on the other: from $[0,1]^3$ to $[0,1]^2$

TODO : we do z-mapping on each slice $(v_2, v_3)$ of $v_1$, and downsample. we end up with a 2d neural field, direction $v_1$ is continuous, direction $\alpha_{23}$ is fractal. now the field is not continuous anymore, it is just sufficiently regular to be L1. this shows "local mappings conserve regularity"

::: {.content-visible when-format="html"}
![(TODO 3D -> 2D only) Equivalence of dynamics, from 3D to 2D.](figures/embedding_3d_2d_1d.mp4)
:::

::: {.content-visible when-format="pdf"}
![(TODO PDF, 3D -> 2D only) Equivalence of dynamics, from 3D to 2D.](figures/lol7.jpg)
:::

![(TODO 3D -> 2D only) Effective sampling points after averaging.](figures/mapping_3d_2d_1d.png){width=400px}

#### From fractal to fractal : $[0,1]^2$ to $[0,1]$

TODO : we can re-apply the z-mapping in $(v_1, \alpha_{23})$, we get a fractal 1d embedding. completely fractal, but the dynamics still work, because the mapping conserves regularity.
...
::: {.content-visible when-format="pdf"}
![(TODO PDF) Equivalence of dynamics, from 3D to 2D to 1D.](figures/lol7.jpg)
:::

![(TODO) Effective sampling points after averaging.](figures/mapping_3d_2d_1d.png){width=400px}