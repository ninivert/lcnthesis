...
Let us for a moment consider another type of model for networks of neurons, that of homogeneous population dynamics, that describes how $N_\text{pop}$ different populations interact with each other. These types of models emerge when networks of neurons exhibit a finite number of "neuron types". We then group similar neurons ad make the approximation that all neurons inside the population have the same potential, and the same interaction weights with other neurons. In effect, every population then represents in actuality an infinity of discrete identical neurons.
...
1. Cantor functions are bijections between $[0,1]$ and $[0,1]^2$. By the non-existence theorem, these functions cannot be continuous.
...
In light of these results, we might be tempted to search for expressions for Cantor-type functions, since having a bijection would allow us to have a direct mapping between the square and the segment populations. Unfortunately, these functions are discontinuous which breaks the regularity of the kernel.

Peano functions seem like good candidates for the mapping $S^{-1}$, and are extensively studied in the literature (we refer for instance to [@sagan_space-filling_1994;@Sagan1993]). They have the nice property that they are continuous and surjective, so we the segment populations can "reach" the entirety of the square populations. All things considered, non-injectivity is not a problem, since it just means that two segment populations can "refer" to the same square population, which does not break the regularity of the kernel, since the two segment populations can just have their own neighborhoods. Furthermore, we will see that in our examples, the non-injectivity can be ignored because the mapping is quasi-bijective.
...
The intuition behind this expansion is we take a segment, split it in two, write 0 to go left, write one to go right, then recurse. Then combine the bits of $v^{(n)}_1$ and $v^{(n)}_2$ to get the position in the $[0,1]^2$ embedding.
...
With this, we can formulate any mapping of $4^n$ distinct elements (here : squares of size $2^{-n} \times 2^{-n}$) onto $4^n$ elements (here : segments of length $4^{-n}$).

The intuition behind the use of $2n$ bits for the image is that we take $2^n \times 2^n = 4^n$ squares in $[0,1]^2$ and map those to $4^n$ segments in $[0,1]$. Informally, we map $n+n$ bits onto $2n$ bits.
...
```{python}
#| label: fig-anti-z
#| fig-cap: Anti-Z mapping for $n=2$. Numbers represent the ordering of the populations in the $[0,1]$ embedding. We don't plot the trace, because it has many crossovers and overlaps which makes the plot hard to read.
#| echo: false
...
The Z-mapping however seems to converge to a fractal curve. Numerically, there seems to be an infinite number of discontinuities, but the size of these discontinuities is overwhelmingly small, and *on average* the size of these numerical discontinuities vanishes as $n\to\infty$. We won't go further into detail about this in this section (in @sec-zcurve-lebesgue we develop the convergence to a space-filling fractal curve), but this gives some intuition for the arguments presented in @sec-locality.
...
In the previous sections, we have discussed what mappings have good properties, and have given a few hints as to how we can implement them numerically. Since the mappings we presented can be described as a binary expansion, they naturally lend themselves to numerical approximations. Furthermore, this is made especially easy since the mappings we defined simply are permutations of bits (in the $n < \infty$ case), and so we can write a one-to-one mapping between (unsigned integers).
