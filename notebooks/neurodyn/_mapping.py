...
# TODO : move the j_coords methods to the new class
# class RecursiveQuadrantMapping2(BinMapping):
...
class RecursiveQuadrantMapping(BinMapping):
	...
	def indices(self, F: np.ndarray, bbox: Box | None = None) -> np.ndarray:
		if bbox is None:
			bbox = Box.new_bbox(F)
		coords = self._j_coords(F, bbox, self.nrec)
		return (coords-1) @ np.logspace(self.nrec-1, 0, num=self.nrec, base=4, dtype=int)

	def _j_coords(self, F: np.ndarray, box: Box | None = None, n: int | None = None) -> np.ndarray:
		"""Generates coordinates (j1, ..., jn) for the recursive quadrant mapping box -> [0,1]
		...
		# call for coordinates of lower layers
		coords[mask1, 1:] = self._j_coords(F[mask1, :], box1, n-1)
		coords[mask2, 1:] = self._j_coords(F[mask2, :], box2, n-1)
		coords[mask3, 1:] = self._j_coords(F[mask3, :], box3, n-1)
		coords[mask4, 1:] = self._j_coords(F[mask4, :], box4, n-1)
...
class FarMapping(BinMapping):
	...
	def indices(self, F: np.ndarray, bbox: Box | None = None) -> np.ndarray:
		if bbox is None:
			bbox = Box.new_bbox(F)
		return self._far_indices(F, bbox, n=self.nrec)

	def _far_indices(self, F: np.ndarray, bbox: Box, n: int, offset: int = 0, stride: int = 1) -> np.ndarray:
		...
		indices[mask1] = self._far_indices(F[mask1, :], box1, n-1, offset=offset+0*stride, stride=stride*4)
		indices[mask2] = self._far_indices(F[mask2, :], box2, n-1, offset=offset+1*stride, stride=stride*4)
		indices[mask3] = self._far_indices(F[mask3, :], box3, n-1, offset=offset+2*stride, stride=stride*4)
		indices[mask4] = self._far_indices(F[mask4, :], box4, n-1, offset=offset+3*stride, stride=stride*4)
		return indices
