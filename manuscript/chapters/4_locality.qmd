...
With this view, we understand that the coarse-grained performed in @sec-coarse-graining was making use of that fact that locality helps by bounding the variation inside of each averaged bin. Therefore, we would like for mathematically formulate that on average, the variation inside of each bin before coarse-graining is small. @fig-sketch-binned-variation gives an intuition of this. Even though the $[0,1]$ embedding might be discontinuous in places, on average nearby neurons should exhibit the same activity levels.
...
The definition of locality can essentially be seen as a weakened version of continuity; since from definition the image of a small neighbourhood through a continuous function is also a small neighbourhood. Therefore, continuity of a function $f$ implies that $V_n(f) \xrightarrow{n \to \infty} 0$. The reverse is obviously not true, since for instance the metric $V_n$ vanishes when applied to a step function, which is only piecewise continuous.
...
![(TODO change title, change name of mappings, remove the theory, but keep upper bound) (Numerator of the) average binned variation as a function of $n$. The local mappings have a growth slower than the upper bound $2\times 2^n$.](figures/v_num.png){width=500px #fig-vnum}
...
In @sec-Vn, we have introduced the average variation metric $V_n$. Assuming that $V_n \xrightarrow{n\to\infty} 0$ (which expresses the locality property), does this imply that the numerical estimation of the integral in the neural field equation in $[0,1]$ converges to its true value?
