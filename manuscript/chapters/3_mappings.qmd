...
#| fig-cap: Square populations corresponding to the segment populations before coarse-graining are colored identically when they are the same bin of 1D length $2^{-n}$. A cross marks the "average position" in each bin. Here we show $n=3$.
#| fig-subcap:
#|   - Z-mapping
#|   - Column mapping
#| layout-ncol: 2
...
def plot_averaging_geom(mapping_cls, nrec=3):
	...
	for i in range(2**nrec):
		c = next(colors)
		ax.scatter(*F_[i, :, :].T, zorder=10, color=c, ec='k')
		ax.scatter(*F_[i, :, :].mean(axis=0), zorder=10, s=80, marker='X', ec='k', color=c)
	ax.set(
		xlim=(0,1), ylim=(0,1),
