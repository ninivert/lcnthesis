...
Early recordings of cats' somatosensory @Mou57 and visual @HubWie62 cortices suggested that the cortical sheet is organized in vertical columns of functionally similar neurons.

The columnar organization of the cortex motivated the design of spatially structured models of neural population dynamics, namely neural field models [@WilCow73;@Nun74;@Ama77], where the spatial dimensions corresponded to the two dimensions of the cortical sheet. 
...
![Every neuron in the neural network has a position in the $p$-dimensional embedding space.](figures/fig-rnn.svg){width=500px}
...
[^1]: Rigorous proofs of convergence are still an active area of study [@CheDua19;@AGATHENERINE202286] and are not the subject of this thesis.

$$
\partial_t h(\vec z, t) = -h(\vec z, t) + \int_{\mathbb{R}^p} w(\vec z, \vec y) \phi(h(\vec y, t)) \rho(\mathrm d \vec y).
$$ {#eq-nf}

In this equation, as well as for the rest of this work, we will use the notation $\rho(\mathrm d \vec y) := \rho(\vec y) \mathrm d \vec y$ to emphasize that the integral is weighted by the neuron density.
...
Informally, these recent results suggest that, as long as weights scale as $1/N$, the dynamics of large networks can be described, as $N \to \infty$, by a neural field equation where the embedding is simply the interval $[0,1]$, which motivates the search for a formulation of a one-dimensional neural embedding.
...
The main idea is to define a measurable bijective mapping $S : \Rp \mapsto [0,1]$ and a measure $\lambda : \Rp \mapsto \mathbb{R}^+$, such that we can write a one-dimensional neural field	$\tilde h(\alpha, t)$ with a connectivity kernel loosely defined as $\tilde w(\alpha, \beta) = w(S^{-1}(\alpha), S^{-1}(\beta))$.
...
![Good mappings from $\Rp$ to $[0,1]$ should maintain some sense of "locality".](figures/fig-locality.svg){width=600px #fig-intuition-locality}
