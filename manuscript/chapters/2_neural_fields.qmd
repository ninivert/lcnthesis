...
```{python}
#| label: fig-spectrum
#| fig-cap: Spectrum of $K$ for $N=64$ and $p=2$. The numerical estimation (black) of the eigenvalues is close to the analytical ($N \to \infty$) derivation (red). Eigenvalues above the dotted gray line(positive values) correspond to unstable fixed points, and below (negative values) correspond to stable fixed points. Only the 
#| fig-subcap:
...
def plot_spectrum(hstar_str: str):
	params = LowRankRNNParams.new_valentin(p=2, N=256, phi=sigmoid, I_ext=zero, exclude_self_connections=False)
	if hstar_str == 'pattern':
		hstar = params.F[:, 0]
	elif hstar_str == 'zero':
	...
	ax.plot(np.sort(np.linalg.eigvals(J).real)[::-1][:8], marker='o', clip_on=False)
	ax.plot([7,8,8.5], np.sort(np.linalg.eigvals(J).real)[::-1][8:11], linestyle='--', color='k', clip_on=False)
	ax.plot(l[::-1], color='r', marker='o', clip_on=False)
	ax.set_ylim((-1, 0.5))
	ax.set_xlim((0, 8.5))
...
![(TODO animation in 3D, rename non-cycling to original, number from one) Intuition for the cycling behavior by considering the recurrent currents](figures/cycling_recurrent.png){#fig-cycling-irec}
...
![(TODO not color) A grid in $[0,1]^2$ and the corresponding samples in $\R^2$. The numeric density of points in $\R^2$ approaches the normal distribution as the grid becomes finer.](figures/sampling_R2_01.png){#fig-sampling-R2-012}
