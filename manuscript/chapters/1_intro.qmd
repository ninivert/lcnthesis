...
Early recordings of the cat's somatosensory visual cortices suggested that the cortical sheet is organized in vertical columns of functionally similar neurons @Mou57.
...
Let us now give a short introduction on the mathematical formulation of neural fields. Neural field equations emerge from the structure of networks of rate neurons, when the number of neurons $N$ becomes large. Positions $\vec z_i \in \Rp$ are attributed to each neuron, which defines the *embedding space*. Each neuron has a potential $h_i(t)$ that evolves in time, and the weight with which the neurons interact with each other is given by the components of the connectivity matrix $J_{ij} \in \R^{N \times N}$. Additionally, the activation function $\phi(h)$ describes the firing rate of a given neuron with potential $h$. Then, the product $J_{ij} \phi(h_j)$ gives the *recurrent current* of neuron $j$ on neuron $i$.
...
If the embedding is well-chosen,[^1] the connectivity matrix $J_{ij}$ approaches a connectivity kernel $w(\vec y, \vec z)$ as $N \rightarrow \infty$, and neurons close in the embedding space tend to have similar potentials, such that we can write the continous *neural field* $h(t, \vec z)$ of neuron potentials. Before we write down the neural field equation which describes how the field evolves in time, we need to introduce the neuron density $\rho(\vec z)$, which measures the number of neurons per unit volume of the embedding space. The neural field equation is then written as such:
...
![(TODO add phi, add z instead of xi) The connectivity kernel defines the influence between neural densities in the embedding space](figures/connectivity_kernel.png)

## Motivation for a one-dimensional neural field

This work was inspired by recent mathematical development in mean-field theories involving the theory of graphons @JabPoy21. Their main result is that under some boundedness assumptions of the connectivity between the components of a system, the equation of evolution incorporates only an integral over $[0,1]$. This is indeed the case in the toy model described next chapter, where weights scale as $J_{ij} \sim \frac 1N$, which motivates the search for a formuation of a one-dimensional neural embedding.

The main idea is to define a measurable bijective mapping $S : \Rp \mapsto [0,1]$ and a measure $\lambda : [0,1]^2 \mapsto \mathbb{R}^+$, such that we can write a one-dimensional neural field	$\tilde h(\alpha, t)$ with a connectivity kernel loosely defined as $\tilde w = w \circ S^{-1}$.
...
$$
\partial_t \tilde h(t, \alpha) = -\tilde h(t, \alpha) + \int_{[0,1]} \tilde w(\alpha, \beta) \phi(\tilde h(t, \beta)) \left[\lambda \circ S^{-1}\right](\d \beta)
$$

This change of variable in the kernel seems counter-intuitive : how is it possible to map a higher dimension to a lower dimension without losing information? And even if mathematically such a change of variables is possible, is it even feasible to simulate it numerically?
