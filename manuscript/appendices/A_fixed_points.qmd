...
# Fixed point study in the network of neurons {#sec-fixedpoints}
...
## Stability of the fixed points {#sec-fixedpoints-stability}
...
h_i^\star=F_{\nu i} &\implies \lambda_\nu = \avg{\tilde\phi(z) \partial \phi(z) z} - 1 \approx -0.28090 ± 0.00006
\end{cases}
\end{aligned}
$$
...
* When $\phi$ is linear, we see that all the eigenvalues are zero, therefore the first order stability is inconclusive. Numerically, we see that when $\phi$ is linear, the convergence to the fixed points is very slow.
...
* When $\phi$ is cubic, the sign of the eigenvalues depends primarily on $c_3$ (negative $c_1$ breaks the assumption that $\phi$ must be monotonically increasing). The intuition is that we require $c_1$ to be large enough (in other words, the growth of the firing rate when $h$ becomes positive) and $c_3 < 0$ such that $3 c_1 c_3 + 15 c_3^2 < 0$.
  The intuition behind this is that if the slope $c_1$ is stronger, then the bump of $\partial \phi$ becomes "more concentrated", which helps lower the eigenvalues associated with the pattern fixed points.
